...
type
    {
    TICEngineProto - абстрактный тип движк.
    }
    TICEngineProto = class(TObject)
    private
      { Менеджер настроек }
    ...
    {
    TICGenerator - Движок ГЕНЕРАТОРА ТЕКСТОВЫХ ФАЙЛОВ из различных источников.
    }
    TICGenerator = class(TICEngineProto)
    private
...
constructor TICEngineProto.Create(TheOwner: TComponent);
begin
  inherited Create;
...
destructor TICEngineProto.Destroy;
begin
  DestroySources();
  DestroyDestinations();
...
function TICEngineProto.InitSettings():Boolean;
var
...
begin
  ...
  log.InfoMsgFmt('INI Файл <%s>', [ini_filename]);
...
function TICEngineProto.RegObject(Obj: TICObjectProto; Objects: TStrDictionary): Boolean;
var
  name: AnsiString;
...
{Регистрация объекта-источника данных}
function TICEngineProto.RegSource(Obj: TICObjectProto): Boolean;
begin
  Result := RegObject(Obj, FSources);
end;

{Регистрация объекта-получателя данных}
function TICEngineProto.RegDestination(Obj: TICObjectProto): Boolean;
begin
...
function TICEngineProto.FindObject(sObjName: AnsiString; Objects: TStrDictionary): TICObjectProto;
begin
...
{ Поиск объекта-источника данных }
function TICEngineProto.FindSource(sObjName: AnsiString): TICObjectProto;
begin
  Result := FindObject(sObjName, FSources);
end;

{ Поиск объекта-получателя данных }
function TICEngineProto.FindDestination(sObjName: AnsiString): TICObjectProto;
begin
...
function TICEngineProto.CreateDataCtrl(Properties: TStrDictionary): TICObjectProto;
var
...
function TICEngineProto.CreateSources(ObjectNames: TStringList): Boolean;
var
  //ctrl_objects: TList;
  obj: TICObjectProto;
...
function TICEngineProto.CreateDestinations(ObjectNames: TStringList): Boolean;
var
  obj: TICObjectProto;
  obj_names_str: AnsiString;
  i: Integer;
...
function TICEngineProto.DestroySources(): Boolean;
begin
  Result := False;
...
function TICEngineProto.DestroyDestinations(): Boolean;
begin
  Result := False;
...
{ Получить состояние тега источника данных в виде строки }
function TICEngineProto.GetSourceStateAsString(aSourceName, aTag: AnsiString): AnsiString;
var
  src: TICObjectProto;
begin
...
//function TICEngineProto.GetSourceTimeStateAsList(aSourceName, aTag: AnsiString): TMemVectorOfString;
//var
//  src: TICObjectProto;
//  i: Integer;
...
constructor TICGenerator.Create(TheOwner: TComponent);
begin
  inherited Create(TheOwner);
  //FIsTick := False;
end;

destructor TICGenerator.Destroy;
begin
  inherited Destroy;
end;

{ Прочитать значение из источника данных }
function TICGenerator.ReadValueAsString(sSrcTypeName: AnsiString; const aArgs: Array Of Const; sAddress: AnsiString): AnsiString;
var
  ctrl_obj: TICObjectProto;
...
{ Прочитать список значений из источника данных }
function TICGenerator.ReadValuesAsStrings(sSrcTypeName: AnsiString; const aArgs: Array Of Const; aAddresses: Array Of String): TStringList;
var
  ctrl_obj: TICObjectProto;
  str_list: TStringList;
...
//procedure TICGenerator.Test;
//begin
//  log.InfoMsg('Режим тестирования службы')
//end;

procedure TICGenerator.WorkTick;
var
  i: Integer;
